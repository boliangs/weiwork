...
func (cache *RDBCache) GetValue(key string) (string, error) {
	result, err := cache.Rdb.Get(context.Background(), key).Result()
	if err == redis.Nil {
		//缓存项不存在进行处理
		return "", err
	} else if err != nil {
...
func (cache *RDBCache) SaveValue(key string, value any, time time.Duration) {
	err := cache.Rdb.Set(context.Background(), key, value, time).Err()
	if err != nil {
		if err != redis.Nil {
...
func (cache *RDBCache) SaveGeSession(openid string, geSession string) (err error) {
	err = cache.Rdb.Set(context.Background(), "hqu"+openid, geSession, 10*time.Minute).Err()
	if err != nil {
...
func (cache *RDBCache) GetECardCertificate(stunum string) (eCardCertificate types.ECardCertificate, err error) {
	date, err := cache.Rdb.HGet(context.Background(), "eCardCertificate", stunum).Result()
	if err != nil {
		if err != redis.Nil {
			logrus.Info("[RDBERROR]:%v\n", err.Error())
		}
...
func (cache *RDBCache) GetJwcCertificate(stunum string) (jwcCertificate types.JwcCertificate) {
	maps, err := cache.Rdb.HGetAll(context.Background(), "jwcCertificate:"+stunum).Result()
	if err != nil {
		if err != redis.Nil {
			logrus.Info("[RDBERROR]:%v\n", err.Error())
		}
...
func (cache *RDBCache) SaveECardCertificate(eCardCertificate types.ECardCertificate, stunum string) (err error) {
	...
	err = cache.Rdb.HSet(context.Background(), "eCardCertificate", stunum, string(data)).Err()
	if err != nil {
		logrus.Info("[RDBERROR]:%v\n", err.Error())
		return
	}
...
func (cache *RDBCache) SaveJwcCertificate(jwcCertificate types.JwcCertificate, stunum string) (err error) {
	...
	err = cache.Rdb.HMSet(context.Background(), "jwcCertificate:"+stunum, maps).Err()
	if err != nil {
		logrus.Info("[RDBERROR]:%v\n", err.Error())
		return
	}
	err = cache.Rdb.Expire(context.Background(), "jwcCertificate:"+stunum, 8*time.Minute).Err()
	if err != nil {
		logrus.Info("[RDBERROR]:%v\n", err.Error())
		return
	}
