...
import (
	...
	"sendswork/app/yearBill/database/model"
	"sendswork/app/yearBill/database/mq"
	"sendswork/app/yearBill/script"
	"sendswork/config"
...
func (*YearBillSrv) InfoCheck(ctx context.Context, req *YearBillPb.InfoCheckRequest) (resp *YearBillPb.InfoCheckResponse, err error) {
	...
	if len(bills) == 0 || bills[0].BestRestaurantPay == 0 {
		return
	}
	//查询缓存
...
func (*YearBillSrv) GetCertificate(ctx context.Context, req *YearBillPb.GetCertificateRequest) (resp *YearBillPb.GetCertificateResponse, err error) {
	...
	go func() {
		...
		openidReq := aes.AesEncoding(req.Openid)
		gssession, errJwc := school.GetGsSession(openidReq)
		if errJwc != nil {
			return
		}
		aes.SetKey(utils.AcquiesceKey3)
		err, gssession = aes.AesDecoding(gssession)
		if err != nil {
			return
		}
		jwc := school.NewJwc()
...
func (*YearBillSrv) DataInit(ctx context.Context, req *emptypb.Empty) (empty *emptypb.Empty, err error) {
	...
	for d := range msgs {
		...
		ctxTask, cancel := context.WithTimeout(context.Background(), 6*time.Second)
		go func() {
			d.Ack(false)
