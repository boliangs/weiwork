...
func (dao *UserDao) CreateUser(stuNum string) (err error) {
	...
	err = dao.WithTableName().Create(&user).
		Error
	if err != nil {
...
func (dao *UserDao) StorageData(stuNum string, index int64, bill model.Bill, learn model.Learn) (err error) {
	...
	if err != nil {
		...
		err = tx.Table("bills").
			Where("stu_num = ?", stuNum).
			Updates(map[string]any{"best_restaurant": bill.BestRestaurant,
				"best_restaurant_pay": bill.BestRestaurantPay,
				"early_time":          bill.EarlyTime,
				"last_time":           bill.LastTime,
				"other_pay":           bill.OtherPay,
				"library_pay":         bill.LibraryPay,
				"restaurant_pay":      bill.RestaurantPay,
			}).Error
		if err != nil {
			tx.Rollback()
