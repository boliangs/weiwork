...
import (
	...
	"fmt"
	"io/ioutil"
	"sendswork/app/school/types"
	"sendswork/utils"
	"sendswork/utils/httpUtil"
	"sync"
...
func (jwc *Jwc) GetEmaphome_WEU() (emaphome_WEU string, err error) {
	...
	if err != nil {
		logrus.Infof("[GetEmaphome_WEU JSONToMap ERROR]: %v\n", err)
		return "", err
	}

	// 确保body中有"data"键，并检查它是否是string类型
	data, ok := body["data"].(string)
	if !ok {
		logrus.Infof("[GetEmaphome_WEU 类型断言错误]: body['data'] 类型断言错误")
		return "", fmt.Errorf("body['data']类型断言错误: %#v", body["data"])
	}
	emaphome_WEU = data
