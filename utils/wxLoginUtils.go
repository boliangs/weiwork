...
func WXLogin(code string) (*WXLoginResp, error) {
	...
	url = fmt.Sprintf(url, config.AppId, config.AppSecret, code)
...
func GetStuNum(openId string) (*GetStuNumResp, error) {
	...
	*/
	...
	//原本http://wx.sends.cc/d46c6Zd6wyEto4gWqyfx/getstuid
	resp, _ := http.Post("http://wx.sends.cc/api/stunum", "application/json", bytes.NewReader(bytesData))
	defer resp.Body.Close()
	// 解析http请求中body 数据到我们定义的结构体中
...
func GetAccessToken() (*GetAccessTokenResp, error) {
	...
	// 合成url, 这里的appId和secret是在微信公众平台上获取的
	url = fmt.Sprintf(url, config.AppId, config.AppSecret)
...
func GenerateSignature(jsapiTicket string, timestamp int64, nonceStr string, url string) string {
	str := fmt.Sprintf("jsapi_ticket=%s&noncestr=%s&timestamp=%s&url=%s", jsapiTicket, nonceStr, strconv.FormatInt(timestamp, 10), url)
